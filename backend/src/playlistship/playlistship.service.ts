...
} from './dto/playlistshipResponse.dto';
...
export class PlaylistshipService {
  ...
  async sendPlaylistInvitation(
    playlistshipDto: PlaylistshipDto,
    senderId: number,
  ): Promise<void> {
    await this.checkOwnership(senderId, playlistshipDto.playlistId);
    if (
      await this.playlistshipExists(
        playlistshipDto.playlistId,
        playlistshipDto.addresseeId,
      )
    ) {
      throw new BadRequestException(
        'User Already has already received an invitation to the playlist',
      );
    }

    const playlist = (await this.playlistsService.findOne({
      id: playlistshipDto.playlistId,
    }))!;
    if (playlist.userId === playlistshipDto.addresseeId) {
      throw new BadRequestException(
        'You can not invite yourself to your playlist',
      );
    }

    if (playlist.isDefault)
      throw new BadRequestException(
        'No Invitation to Favorite Playlist allowed',
      );
    ...
    const playlistName = (
      await this.playlistsService.findOne({ id: playlistshipDto.playlistId })
    )?.title;
  ...
  async createPlaylistship(
    data: Prisma.playlistshipUncheckedCreateInput,
    ...
    let playlistship = await this.playlistship({
      playlistId_addresseeId: {
        playlistId: data.playlistId,
        addresseeId: data.addresseeId,
      },
    });
    if (playlistship) return playlistship;
  ...
  async playlistship(
    data: Prisma.playlistshipWhereUniqueInput,
  ): Promise<playlistship | null> {
    return await this.prisma.playlistship.findUnique({
      where: data,
    });
  }

  async playlistshipExists(
    playlistId: number,
    addresseeId: number,
  ): Promise<boolean> {
    let playlistship = await this.playlistship({
      playlistId_addresseeId: { playlistId, addresseeId },
    });
  ...
  async updatePlaylistshipStatus(
    ...
    if (status === invitationStatus.REJECTED) {
      await this.deletePlaylistship({ playlistId, addresseeId }, addresseeId);
      return {
        playlistId,
        addresseeId,
        status,
  ...
  async updateManpyPlaylistshipStatus(
    where: Prisma.playlistshipWhereInput,
    data: Prisma.playlistshipUpdateInput,
  ) {
  ...
  async answerPlaylistInvitation(
    playlistshipDto: PlaylistshipAnswerDto,
    userId: number,
  ): Promise<playlistship> {
  ...
  async checkOwnership(userId: number, playlistId: number) {
    ...
    if (playlist?.userId != userId)
      throw new UnauthorizedException(
        'You do not own or have access to this playlist!',
      );
  }
}
